...
#define LOG_HEAD_LOG_SERVER  "PercipioTcpLogServer"
...
bool PercipioTcpLogServer::start(int port) {
    if (running_) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Server is already running on port " << port_);
        return false;
    }
    ...
    if (server_fd_ < 0) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Failed to create socket: " << strerror(errno));
        return false;
    }
    ...
    if (setsockopt(server_fd_, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Failed to set socket options: " << strerror(errno));
        close(server_fd_);
        return false;
    }
    ...
    if (bind(server_fd_, (struct sockaddr*)&address, sizeof(address)) < 0) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Failed to bind socket: " << strerror(errno));
        close(server_fd_);
        return false;
    }
    ...
    if (listen(server_fd_, 10) < 0) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Failed to listen on socket: " << strerror(errno));
        close(server_fd_);
        return false;
    }
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "TCP Log Server started on port " << port_ );
    return true;
}
...
void PercipioTcpLogServer::stop() {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "TCP Log Server stopped");
}
...
void PercipioTcpLogServer::run() {
    ...
    while (running_) {
        ...
        if (poll_count < 0) {
            ...
            RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Poll error: " << strerror(errno));
            break;
        }
        ...
        if (poll_fds[0].revents & POLLIN) {
            ...
            if (client_fd < 0) {
                ...
                RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Failed to accept connection: " << strerror(errno));
                continue;
            }
...
void PercipioTcpLogServer::handleClient(int client_fd, const std::string& client_ip, int client_port) {
    ...
    close(client_fd);
    RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_LOG_SERVER), "Client disconnected(" << client_ip << ":" << client_port << ")");
}
