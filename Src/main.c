/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : 拖拉机改装遥控控制主程序 (5步科电机 + 1方向盘舵机 + 1推杆继电器 + 1倾角传感器)
...
int main(void)
{
    ...
    /* 5. 同步电机出厂速度与加减速度参数 */
    kinco_motor_config_sync(&hcan1, &g_motor_gearshift2, 200,  50, 50);
    ...
    while (1)
    {
        ...
        if (now - print_tick >= 200)
        {
            ...
            if (huart1.gState == HAL_UART_STATE_READY)
            {
                ...
                long roll_x100  = (long)(g_inclinometer.roll_deg * 100.0f);
                long pitch_x100 = (long)(g_inclinometer.pitch_deg * 100.0f);
                long comp_x10   = (long)(g_steer_closed_loop_adj_deg * 10.0f);
                uint8_t inc_ok  = Inclinometer_IsOnline();
                ...
                int len = snprintf(tx_buf, sizeof(tx_buf),
                    ...
                    "[INC 0x18B] On:%d Roll:%ld.%02ld Pitch:%ld.%02ld Comp:%ld.%01ld\r\n\r\n",
                    ...
                    inc_ok, roll_x100 / 100, labs(roll_x100 % 100), pitch_x100 / 100, labs(pitch_x100 % 100), comp_x10 / 10, labs(comp_x10 % 10)
                );
