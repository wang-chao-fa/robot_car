...
typedef struct {
    /* --- 控制指令字段 --- */
    uint8_t  gear;              // 档位指令: 0: 空档/原点(0), 1: 前进档(+1600000), 2: 倒车档(-1600000)
    uint8_t  clutch;            // 离合指令: 0: 结合/释放(0), 1: 踩下/分离(-160000)
    uint8_t  throttle_percent;  // 油门指令开度: 0 ~ 100 (%)
    uint8_t  brake;             // 刹车指令: 0: 松开(0), 1: 踩下刹车(-160000)
    int8_t   actuator_dir;      // 继电器推杆方向: -1: 收回, 0: 停止, 1: 伸出
    float    steer_speed_rpm;   // 方向盘转向转速: (-100.0 ~ +100.0 RPM)
